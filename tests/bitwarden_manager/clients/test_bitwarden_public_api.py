...
def test_update_collection_external_id_to_encoded_team_name() -> None:
    ...
    with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        client.update_collection_external_id_to_encoded_team_name(collection_object, teams)

        rsps.assert_call_count(f"https://api.bitwarden.com/public/collections/{team_one_id}", 2) is True
        assert rsps.calls[1].response.status_code == 200
        assert json.loads(rsps.calls[1].request.body) == {"externalId": team_one_external_id_encoded, "groups": []}


def test_update_collection_external_id_to_encoded_team_name_failure() -> None:
    team_one_name = "Team One"
    team_one_id = "id-team-one"
    team_one_external_id_encoded = _external_id_base64_encoded(team_one_name)
    collection_object = {"id": team_one_id, "externalId": team_one_name, "groups": []}
    ...
    with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        with pytest.raises(Exception, match=f"Failed to update external id of collection: {team_one_id}"):
            client.update_collection_external_id_to_encoded_team_name(collection_object, teams)


def test_update_all_team_collection_external_ids() -> None:
    team_one_name = "Team One"
    team_one_id = _collection_id(team_one_name)
    team_one_external_id_encoded = _external_id_base64_encoded(team_one_name)
    ...
    with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
        ...
        rsps.add(
            ...
            match=[
                matchers.json_params_matcher(
                    {
                        "externalId": team_two_external_id_encoded,
                        "groups": [],
                    }
                )
            ],
        ...
        client.update_all_team_collection_external_ids(teams)

        put_call_request_bodies = [json.loads(c.request.body) for c in rsps.calls if c.request.method == "PUT"]

        assert {"externalId": team_one_external_id_encoded, "groups": []} in put_call_request_bodies
        assert {"externalId": team_two_external_id_encoded, "groups": []} in put_call_request_bodies
